...
def main():
    ...
    parser.add_argument("-p", "--pair", default='notre_dame', help="Either notre_dame, mt_rushmore, or e_gaudi. Specifies which image pair to match")
    ...
    # points for mt. rushmore will not produce good r
    # esults, so you'll have to use
    ...
    # dxx=ndimage.convolve(image1, k, mode='constant', cval=0.0)*0.25
    # dyy=ndimage.convolve(image1, k, mode='constant', cval=0.0)*0.25
    # dx = ndimage.sobel(image1,1)
    # dy = ndimage.sobel(image1,0)
    # root= np.sqrt(dx**2+dy**2)
